...
onready var magicUIFullRed = $MagicUIFullRed
...
func _ready():
	...
	if players == 1:
		...
		magicUIFull.rect_position = Vector2(P1_MAGIC_POSITION_X + 1, P1_MAGIC_POSITION_Y)
		...
		magicUIFullRed.rect_position = Vector2(P2_MAGIC_POSITION_X + 1, P2_MAGIC_POSITION_Y)

func _physics_process(delta):
	magicUIFull.rect_size.x = move_toward(magicUIFull.rect_size.x, size, ANIMATION_SPEED * delta)
	if players == 2:
		magicUIFullRed.rect_size.x = move_toward(magicUIFullRed.rect_size.x, red_size, ANIMATION_SPEED * delta)
